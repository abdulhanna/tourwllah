...
export function buildLehWhatsAppMessage(pkg) {
  const highlights = (pkg.highlights || [])
    .slice(0, 4)
    .map(h => `✔ ${typeof h === 'string' ? h : h.label}`)
    .join('\n')

  return `🏔 Tripcart Holidays — Leh Ladakh Enquiry

📍 ${pkg.title}
🗓 ${pkg.duration}
🛣 ${pkg.route || 'Leh circuit'}
${pkg.price ? `\n💰 ${pkg.price}` : ''}

✨ Highlights:
${highlights}

Please share availability and best price for my dates!`
}

export function openWhatsAppLeh(pkg, phone = DEFAULT_PHONE) {
  const text = encodeURIComponent(buildLehWhatsAppMessage(pkg))
  window.open(`https://wa.me/${phone}?text=${text}`, '_blank', 'noopener,noreferrer')
}
