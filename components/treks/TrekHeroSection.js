...
import { openWhatsAppTrek } from '@/lib/whatsapp'
import Breadcrumb from '@/components/ui/Breadcrumb'

export default function TrekHeroSection({ trek }) {
  return (
    <section className="relative min-h-[600px] lg:min-h-[680px] text-white overflow-hidden flex items-end">
      <Image
        src={trek.heroImage}
        alt={trek.heroHeading}
        fill
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 pb-14">
        <Breadcrumb
          ...
          items={[
            ...
            { label: trek.shortTitle, href: `/treks/${trek.slug}` },
          ]}
        />
        ...
        <div className="mt-6 max-w-3xl">
          <div className="inline-flex items-center gap-2 bg-white/15 backdrop-blur-md text-white text-xs font-semibold px-4 py-1.5 rounded-full mb-5 uppercase tracking-widest border border-white/20">
            🥾 Tripcart Holidays · Himalayan Trek
          </div>
          <h1 className="font-display text-4xl sm:text-5xl lg:text-6xl font-bold leading-tight mb-4 drop-shadow-lg">
            {trek.heroHeading}
          </h1>
          <p className="text-lg sm:text-xl text-white/85 mb-6 leading-relaxed">{trek.heroSubheading}</p>
        ...
        {trek.stats && (
          <div className="mt-12 grid grid-cols-2 sm:grid-cols-4 gap-3 max-w-2xl">
            {trek.stats.map((stat, i) => (
              <div
                key={i}
                className="bg-white/10 backdrop-blur-md rounded-2xl p-4 text-center border border-white/20"
              >
                <div className="font-display text-2xl sm:text-3xl font-bold text-white drop-shadow">
                  {stat.value}
                </div>
